...
class ProposeDistribution:
    ...
    def fit(self, samples, weights=None, iter=5, learning_rate=0.05, perturb=0.05):
        if weights is None:
...
class PQCGroverOptimizer(OptimizationAlgorithm):
    """Uses Grover Adaptive Search (GAS) to find the minimum of a QUBO function."""

    def __init__(
        self,
        num_value_qubits: int,
        propose_dist: ProposeDistribution,
        ...
        self.positive_conf = positive_train_conf
        self.negative_conf = negative_train_conf
    ...
    def _get_a_operator(self, qr_key_value, problem):
        ...
        a_operator.compose(self.propose_distribution.circuit.bind_parameters(self.propose_distribution.param), inplace=True)
    ...
    def solve(self, problem: QuadraticProgram) -> OptimizationResult:
        ...
        while not optimum_found:
            ...
            for n_sample in range(self.sample_num):
                ...
                problem_.objective.constant = orig_constant - self._threshold
                a_operator = self._get_a_operator(qr_key_value, problem_)

                # Iterate until we measure a negative.
                loops_with_no_good_sample = 0
                while not good_sample_found and not optimum_found:
                    # Determine the number of rotations.
                    loops_with_no_good_sample += 1
                    ...
                    k_bin = np.array([int(c, 2) for c in outcome[0:n_key]])
                    k = int(outcome[0:n_key], 2)
                    v = outcome[n_key : n_key + n_value]
                    int_v = self._bin_to_int(v, n_value) + self._threshold
                    ...
                    #     samples, _ = self._interpret_samples(problem, raw_samples, self._converters)
                        ...
                        if (
                            loops_with_no_good_sample >= self._n_iterations
