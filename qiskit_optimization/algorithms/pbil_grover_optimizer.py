...
class PBILGroverOptimizer(OptimizationAlgorithm):
    ...
    def __init__(
        ...
        self._pbil_param = None
        self._threshold = init_threshold
        self.learning_rate = lr
        self.quantile = quantile
        self.pbil_sample_num = pbil_sample_num
    ...
    def solve(self, problem: QuadraticProgram) -> OptimizationResult:
        ...
        while not optimum_found:
            ...
            if not optimum_found:
                # update parameter
                self._pbil_param = self._pbil_param * (1-self.learning_rate) + \
                    self.learning_rate * np.mean(np.array(good_samples), axis=0)
                # TODO: importance sampling
                self._threshold = int(np.quantile(good_sample_values, self.quantile))
                print("param", self._pbil_param)
